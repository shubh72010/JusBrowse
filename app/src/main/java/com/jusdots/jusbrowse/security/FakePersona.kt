...
data class FakePersona(
    ...
    /**
     * Converts to AndroidX UserAgentMetadata for modern WebView support
     */
    fun toUserAgentMetadata(): UserAgentMetadata {
        return UserAgentMetadata.Builder()
            .setBrandVersionList(brands.map { 
                UserAgentMetadata.BrandVersion.Builder()
                    .setBrand(it.brand)
                    .setFullVersion(it.version + ".0.0.0")
                    .setMajorVersion(it.version)
                    .build() 
            })
            .setPlatform(platform)
            .setPlatformVersion(platformVersion)
            .setArchitecture("arm") // Standard for Android
            .setModel(model)
            .setMobile(mobile)
            .build()
    }
